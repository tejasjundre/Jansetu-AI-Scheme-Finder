from django.urls import path

from . import views


urlpatterns = [
    path("healthz/", views.healthz, name="healthz"),
    path("", views.home, name="home"),
    path("register/", views.citizen_register, name="citizen_register"),
    path("login/", views.citizen_login, name="citizen_login"),
    path("logout/", views.citizen_logout, name="citizen_logout"),
    path("dashboard/", views.citizen_dashboard, name="citizen_dashboard"),
    path("profile/", views.citizen_profile, name="citizen_profile"),
    path("applications/", views.citizen_applications, name="citizen_applications"),
    path("applications/new/", views.citizen_application_new, name="citizen_application_new"),
    path("documents/", views.citizen_documents, name="citizen_documents"),
    path("wizard/", views.wizard, name="wizard"),
    path("chat/", views.chat, name="chat"),
    path("schemes/", views.schemes, name="schemes"),
    path("scheme/<slug:slug>/", views.scheme_detail, name="scheme_detail"),
    path("support/", views.support, name="support"),
    path("privacy/", views.privacy, name="privacy"),
    path("ops/", views.ops_dashboard, name="ops_dashboard"),
    path("api/chat/", views.chat_api, name="chat_api"),
    path("api/help-center/", views.help_center_api, name="help_center_api"),
    path("api/speech-to-text/", views.speech_to_text_api, name="speech_to_text_api"),
    path("api/text-to-speech/", views.text_to_speech_api, name="text_to_speech_api"),
]
